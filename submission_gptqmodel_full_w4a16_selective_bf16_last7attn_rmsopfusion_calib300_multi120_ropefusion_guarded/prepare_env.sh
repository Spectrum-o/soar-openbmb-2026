...
# RMS + GUARDED RoPE OP-FUSION OVERLAY.
#
# This overlay removes the Python-side q/k fp32 materialization around RoPE.
# It is only valid when prepare_model.sh first proves bitwise equivalence against
# the current MiniCPM fp32-upcast path for the platform INPUT config.json. If the
# model shape or kernel path is not supported, preparation fails before GPTQ.
if [ -f "${SUBMISSION_DIR}/overlay/minicpm.py" ]; then
    ...
    if [ -f "${TARGET_MINICPM}" ]; then
        echo "[prepare_env] applying guarded RMS+RoPE op-fusion overlay: ${TARGET_MINICPM}"
        cp -v "${SUBMISSION_DIR}/overlay/minicpm.py" "${TARGET_MINICPM}"
        if ! python3 -c "import sglang.srt.models.minicpm; print('[rms_rope_opfusion_overlay] import OK')"; then
            echo "[prepare_env] FATAL: guarded RMS+RoPE op-fusion overlay broke sglang.srt.models.minicpm import" >&2
            exit 1
        fi
    else
        echo "[prepare_env] FATAL: target minicpm.py not found at ${TARGET_MINICPM}" >&2
        exit 1
    fi
fi
