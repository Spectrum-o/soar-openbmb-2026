...
#   - Expected normal time on RTX PRO 6000 (96GB): 45-75 min for this
#     NUM_CALIB=300 + multi-adaptive probe on the SOAR platform's
...
#       prepare_env:   ~ 5 min
#       quantize:      ~45-75 min (heavier multi-adaptive calibration)
#       bench S1+8+max:~95 min (matches the 2026-05-19 RTN runs)
#       eval_model.py: ~15 min
#       TOTAL:        ~160-190 min ≈ 2.7-3.2 h  (still under 5h)
...
ROPE_FUSION_PROBE="${ROPE_FUSION_PROBE:-1}"
ROPE_FUSION_PROBE_TOKENS="${ROPE_FUSION_PROBE_TOKENS:-1,2,7,16,64,257,1024}"
if [ "${ROPE_FUSION_PROBE}" = "1" ]; then
    ROPE_PROBE="${SCRIPT_DIR}/probe_minicpm_rope_fusion.py"
    if [ ! -f "${ROPE_PROBE}" ]; then
        echo "[prepare_model] FATAL: RoPE fusion probe missing at ${ROPE_PROBE}" >&2
        exit 1
    fi
    if [ ! -f "${INPUT_DIR}/config.json" ]; then
        echo "[prepare_model] FATAL: input config.json missing; cannot validate MiniCPM-SALA RoPE shape" >&2
        exit 1
    fi
    echo "[prepare_model] step 0: strict MiniCPM-SALA RoPE no-upcast equivalence probe"
    python3 "${ROPE_PROBE}" \
        --model-config "${INPUT_DIR}/config.json" \
        --dtype bfloat16 \
        --tokens "${ROPE_FUSION_PROBE_TOKENS}" \
        --json
else
    echo "[prepare_model] FATAL: ROPE_FUSION_PROBE=0 is not allowed for guarded RoPE package" >&2
    exit 1
fi
