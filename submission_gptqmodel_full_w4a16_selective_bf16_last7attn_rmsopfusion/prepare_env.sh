...
# RMS-ONLY OP-FUSION OVERLAY (last7attn_rmsopfusion variant).
#
# This intentionally keeps the current RoPE fp32 upcast unchanged. The previous
# opfusion package changed both RoPE dtype handling and the residual/RMSNorm
# path, then collapsed on platform. This variant isolates the residual-delay
# RMSNorm fusion on top of the platform-proven full-selective last7attn route.
if [ -f "${SUBMISSION_DIR}/overlay/minicpm.py" ]; then
    ...
    if [ -f "${TARGET_MINICPM}" ]; then
        echo "[prepare_env] applying RMS-only op-fusion overlay: ${TARGET_MINICPM}"
        cp -v "${SUBMISSION_DIR}/overlay/minicpm.py" "${TARGET_MINICPM}"
        if ! python3 -c "import sglang.srt.models.minicpm; print('[rms_opfusion_overlay] import OK')"; then
            echo "[prepare_env] FATAL: RMS-only op-fusion overlay broke sglang.srt.models.minicpm import" >&2
            exit 1
        fi
    else
