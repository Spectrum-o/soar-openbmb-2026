...
# v2: chunked-prefill bumped 8K -> 32K (verified +59% throughput / -60% TTFT
# on the 5/14 local bench). Must pair with --max-prefill-tokens 32768 because
# sglang has a hidden default of 16384 that otherwise caps effective prefill.
export GPTQMODEL_MARLIN_USE_FP32="${GPTQMODEL_MARLIN_USE_FP32:-1}"
export SGLANG_SERVER_ARGS="--disable-radix-cache --attention-backend minicpm_flashinfer --chunked-prefill-size 32768 --max-prefill-tokens 32768 --skip-server-warmup --dense-as-sparse --quantization gptq_marlin --dtype float16"
