...
# python 3.10, Blackwell-class GPU. Prefer any bundled `flash_attn-*.whl`;
# the current package carries a cp310/cu128/sm120 wheel and installs it in
# seconds with no source compile. The direct URL below is only a last-resort
# fallback if the local wheel is missing.
...
LOCAL_FLASH_ATTN_WHEEL="$(
    find -L "${SUBMISSION_DIR}" -maxdepth 1 -type f -name 'flash_attn-*.whl' | sort | tail -n 1
)"
    ...
    if [ -n "${LOCAL_FLASH_ATTN_WHEEL}" ]; then
        ...
        if ! uv pip install --no-deps --no-build-isolation "${FLASH_ATTN_WHEEL}"; then
            if [ "${ALLOW_FLASH_ATTN_SOURCE_BUILD:-0}" = "1" ]; then
                ...
                echo "[prepare_env]        include a cp310/cu128 flash_attn-*.whl in the submission package" >&2
                exit 1
            fi
        fi
    fi
...
#
# Full-W4A16 inherits the verified BF16 sparse-backend boundary: no fp16
# sed-patch, serve with --dtype bfloat16, and let Marlin handle its internal
# low-level GEMM dtype. Re-enable the sed-patch only as a deliberate ablation.
echo "[prepare_env] full-w4a16: SKIPPING the fp16 sed-patch (BF16 sparse backend)"
...
# SGLang server args for the first full-W4A16 accuracy-recovery pass.
# Keep BF16 KV cache here: FP8KV is a separate follow-up variable after the
# full BF16-KV artifact proves acc >= 80. The chunk32k + mem-fraction 0.70
# stack is the platform-verified safe runtime baseline; avoid chunk65k or
# mem-fraction 0.80 until correctness is established.
