...
1. **Registers MiniCPM-SALA explicitly for FULL W4A16.** SALA's HF
   `model_type` is `minicpm_sala`, which is NOT in GPTQModel's built-in
   registry. This experimental variant registers a `MiniCPMSALAGPTQ`
   subclass of `BaseQModel` with both attention/Lightning q/k/v/o
   projections and MLP gate/up/down projections in `layer_modules`.
   Optional gates (`o_gate`, `z_proj`) stay BF16 because they are not
   present on every SALA layer.
...
4. **No FP8 KV cache in companion prepare_env.sh** for this first full
   quantization experiment. Keep KV BF16/FP16 so any accuracy movement is
   attributable to full W4A16, not KV quantization.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="How to slice prompts into calibration samples. "
             ...
             "sample count — keep --num-calib <= 768 to stay under the "
             "120-min prepare_model timeout.",
    )
    parser.add_argument(
        "--no-chat-template",
        action="store_true",
...
def register_minicpm_sala_with_gptqmodel() -> None:
    ...
    class MiniCPMSALAGPTQ(BaseQModel):
        ...
        FULL W4A16 experiment: quantize common attention/Lightning q/k/v/o
        projections plus MLP gate/up/down. Optional SALA gates such as
        self_attn.o_gate and self_attn.z_proj stay BF16 because they are not
...
def validate_gptq_wrapper(model: Any) -> None:
    ...
    # Full W4A16 intentionally includes self_attn.q/k/v/o. Keep guarding
    # only the optional SALA modules that are not present on every layer.
    forbidden = [
        "o_gate",
        "z_proj",
        "q_norm",
    ...
    if found:
        raise RuntimeError(
            "GPTQModel is still using a template with optional/non-linear SALA "
            f"modules {found}; registration did not override the active MODEL_MAP"
        )
...
def tokenize_calibration(
    ...
    ``window_mode``:
      ...
      - ``"multi-adaptive"``: 1-3 non-overlapping ``max_len``-sized windows
        ...
        count for long prompts — keep ``--num-calib`` <= 768 to stay under
        the 120-min prepare_model timeout.
    """
