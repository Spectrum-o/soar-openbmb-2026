...
# SKIPPED: fp16 sed-patch (Path 1b verified — v5j_dtype_bf16 platform
# 2026-05-24 02:29: acc_ori=82.18, final_score=23.18).
# SGLang's GPTQMarlinLinearMethod casts Marlin fp16 GEMM output to bf16
# when --dtype bfloat16 is set, so the sparse backend can stay native bf16.
# Removing this sed-patch was REQUIRED for the W4A16 path to work; keeping
# it (as the original fp16-targeted lightning_skip variant did) causes
# RuntimeError dtype mismatch at infllm_cuda.varlen_fwd_stage1.
echo "[prepare_env] lightning_skip_bf16: SKIPPING fp16 sed-patch (Path 1b)"
...
export SGLANG_SERVER_ARGS="--disable-radix-cache --attention-backend minicpm_flashinfer --chunked-prefill-size 8192 --skip-server-warmup --dense-as-sparse --quantization gptq_marlin --mem-fraction-static 0.80 --dtype bfloat16"
