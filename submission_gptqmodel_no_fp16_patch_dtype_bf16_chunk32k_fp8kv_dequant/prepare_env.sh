...
# ----------------------------------------------------------------------
# PATH Y — DEQUANT BYPASS (Blackwell FP8 KV without Hopper FA cubin)
# ----------------------------------------------------------------------
# The fp8kv variant used minicpm_flashattn + --kv-cache-dtype fp8_e5m2, which
# routes attention through sgl_kernel's flash_attn_with_kvcache fp8 path.
# That path needs the Hopper-only (SM 9.0) cubin and crashes on Blackwell
# SM 12.0 with "no kernel image is available" at flash_fwd_launch_template.h:166.
#
# Path Y rewrites minicpm_backend.py so:
#   - Q stays in bf16 (no Q to fp8 cast)
#   - K/V cache fetched from paged pool gets dequanted to q.dtype immediately
#   - Standard bf16 FA path runs end-to-end (works on Blackwell)
#
# What we keep: KV POOL storage is still fp8 (memory_pool.py:666 stores as
# uint8). That's the main memory benefit — 2x KV capacity for long context.
# What we lose: fp8 attention bandwidth (KV gets up-cast on read).
#
# Compared to Path X (FlashInfer + ENABLE_SM120): Path Y is safer (no JIT,
# no SM 12.0 cubin gen, no backend swap) but slightly slower at decode.
TARGET_BACKEND="${SUBMISSION_DIR}/sglang/python/sglang/srt/layers/attention/minicpm_backend.py"
DEQUANT_TOOL="${SUBMISSION_DIR}/apply_fp8kv_dequant_bypass_patch.py"
if [ -f "${TARGET_BACKEND}" ] && [ -f "${DEQUANT_TOOL}" ]; then
    echo "[prepare_env] applying Path Y dequant-bypass patch to minicpm_backend.py"
    if ! python3 "${DEQUANT_TOOL}" "${TARGET_BACKEND}"; then
        echo "[prepare_env] FATAL: Path Y dequant-bypass patch failed" >&2
        exit 1
    fi
else
    echo "[prepare_env] WARN: cannot apply Path Y dequant-bypass patch — target or tool missing" >&2
fi
