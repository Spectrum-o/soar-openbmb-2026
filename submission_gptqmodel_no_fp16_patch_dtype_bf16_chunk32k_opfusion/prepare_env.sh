...
# OP-FUSION OVERLAY (v5j_dtype_bf16_chunk32k_opfusion variant):
# Apply the perf/op-fusion branch's minicpm.py over the editable-installed sglang.
# The patch does TWO things (commit 79b0c20f4):
#   1. Drop the float() upcast around rotary_emb in attention layers
#      (apply_rope already runs fp32 internally; upcast is redundant work)
#   2. Adopt residual-delay pattern so RMSNorm dispatches through sglang's
#      fused add_rmsnorm kernel
# Pure-torch CPU equivalence verified (commit 87c22ec88). GPU correctness
# never verified end-to-end with SALA's specific kernels — if acc drops
# materially vs the no-overlay base, op-fusion is numerically wrong on
# this platform's GPU+CUDA combo. Fall back to chunk32k (no overlay).
