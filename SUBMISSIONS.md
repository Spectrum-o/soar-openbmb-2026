...
| 2026-05-23 | 22:30 | **`soar_gptqmodel_no_fp16_patch_v5j_memfrac_fix_20260523_222159.tar.gz`** (v5j memfrac fix) | ⏳ submitted | (pending) | (pending) | **v5j = 1849 prepare_env − fp16 sed-patch + explicit `--mem-fraction-static 0.80`.** Single-variable test of Path 1 hypothesis: is the fp16 sed-patch (which forces lightning attention internal bf16 → fp16) the ONLY blocker for W4A16 + Marlin? If yes, this should match v5g's acc_ori 83.04 while retaining W4A16's 4× decode bandwidth advantage. mem-fraction-static was implicit (default ~0.88) in the earlier 08:33 packing → repacked with 0.80 explicit to match 1849's verified-safe budget (5GB model + 0.80×84 KV + 0.7GB 8K buffer = 72.7GB ≤ 84GB platform, 11GB headroom). md5 e5ded64c0175f99f5220180f6dea6425. Awaiting platform result (~5h ETA). |
