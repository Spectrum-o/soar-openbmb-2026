...
| 2026-05-25 | 23:46 | `submission_gptqmodel_no_fp16_patch_dtype_bf16_chunk32k_fp8kv_flashinfer` (Path X — flashinfer SM120 rebuild + minicpm_flashinfer backend) | 🔴 server start failed | 21m | — | **Path X REFUTED.** Tested hypothesis "rebuild flashinfer with `FLASHINFER_CUDA_ARCH_LIST=12.0f` + nuke ~/.cache/flashinfer + switch attention backend to `minicpm_flashinfer`" (commit `f02334285`). Platform crashed at CUDA-graph capture with `flashinfer/jit/attention/modules.py:978 AssertionError: fp8 tensor core is not supported in fa2 backend`. The assertion fires in flashinfer's JIT module-gen step BEFORE any cubin loads — i.e. before SM 12.0 vs SM 9.0 even matters. Conclusion: **flashinfer's FA2 backend has no FP8 path on any arch**; only FA3 supports FP8, and FA3 in flashinfer is currently Hopper-only. This corrects the 2026-05-25 "Blackwell binary missing" claim — the real blocker is one level up at the backend-selector layer, not the cubin level. Both attention backends SALA can use (`minicpm_flashattn` → sgl_kernel hopper FA; `minicpm_flashinfer` → flashinfer FA2) refuse FP8 for independent reasons. Full analysis in `experiments/FP8KV_PATH_D_INVESTIGATION.md` "Path X update" section. **Stop iterating on flashinfer-rebuild or sgl_kernel SM12.0 paths**; track upstream flashinfer FA3 sm_120 work instead. |
