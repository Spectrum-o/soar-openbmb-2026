...
| 2026-05-26 | night | `soar_gptqmodel_full_w4a16_20260526_0239.tar.gz` (Full-attn W4A16 retry under current fix stack, md5 `f64bdef05ae65311c06b90d667e7a4a4`) | 🟡 submitted, awaiting platform | TBD | TBD | **Full-W4A16 retry under current fix stack (cherry-picked from `exp/full-w4a16` without merging the branch).** Variant: `submission_gptqmodel_full_w4a16/`. Quantization scope: MLP gate/up/down + `self_attn.{q,k,v,o}_proj`; dynamic skips for SALA-specific `self_attn.{o_gate,z_proj,q_norm,k_norm,o_norm}` (not present on every layer). prepare_env.sh = real-file copy of chunk32k_safe's hardened version (transformers 4.57.1 force-reinstall + hub cascade + import smoke + GPTQModel 7.0.0 pin + qzeros fix + tokenizer overwrite). Server args = chunk32k_safe verbatim (minicpm_flashattn + bf16 + chunked-prefill 32K + mem-fraction 0.70, NO fp8 KV). **Historical context**: v17/v22 full-attn platform=0 predates this fix stack — codex's hypothesis is qzeros hardening + tokenizer overwrite + GPTQModel 7.0.0 pin + transformers pin + bf16 native (vs the broken fp16 sed-patch era) should now make this acc-survivable. **Risks**: attention proj quantization historically -3~-5pp vs bf16, may push acc toward 80-gate. Quantization wall-time on platform ~30-60 min (vs MLP-only ~20 min); `QUANT_TIMEOUT_MIN=90` should cover it. Preflight 14/14 PASS · latent-assert lint OK · pack-check OK. Expected: final_score 19-26 (baseline 22.90), ~21 mean; upside 25-28 if acc holds + attention-GEMM Marlin acceleration realizes. Commits: cherry-pick `36dee8aca`, prepare_env symlink→real-file fix `bd6d9fc24`. |
