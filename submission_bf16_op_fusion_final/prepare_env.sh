...
# v6 (op-fusion overlay) — applies the perf/op-fusion branch's minicpm.py
# over the editable-installed sglang. Editable install means the runtime
# reads Python files from ${SUBMISSION_DIR}/sglang/python/sglang/srt/models/minicpm.py,
# so a simple `cp` of the overlay replaces it.
#
# The patch does TWO things (commit 79b0c20f4 on perf/op-fusion):
#   1. Drop the float() upcast around rotary_emb in MiniCPMAttention and
#      MiniCPMLightningMixer (apply_rope already runs fp32 internally).
#   2. Adopt the residual-delay pattern so RMSNorm dispatches through
#      sglang's fused add_rmsnorm kernel.
#
# Pure-torch CPU equivalence verified by commit 87c22ec88. GPU correctness
# was NEVER verified end-to-end against MiniCPM-SALA's specific kernels.
# If acc drops materially vs v5e, op-fusion is numerically wrong on the
# platform's GPU+CUDA combo — fall back to v5e (no overlay).
if [ -f "${SUBMISSION_DIR}/overlay/minicpm.py" ]; then
    TARGET_MINICPM="${SUBMISSION_DIR}/sglang/python/sglang/srt/models/minicpm.py"
    if [ -f "${TARGET_MINICPM}" ]; then
        echo "[prepare_env] applying op-fusion overlay: ${TARGET_MINICPM}"
        cp -v "${SUBMISSION_DIR}/overlay/minicpm.py" "${TARGET_MINICPM}"
        # Verify the overlay imports cleanly
        if ! python3 -c "import sglang.srt.models.minicpm; print('[op_fusion_overlay] import OK')"; then
            echo "[prepare_env] FATAL: op-fusion overlay broke sglang.srt.models.minicpm import" >&2
            exit 1
        fi
    else
        echo "[prepare_env] WARN: target minicpm.py not found at ${TARGET_MINICPM} — overlay skipped" >&2
    fi
fi
...
export SGLANG_SERVER_ARGS="--disable-radix-cache --attention-backend minicpm_flashinfer --chunked-prefill-size 32768 --max-prefill-tokens 32768 --skip-server-warmup --dense-as-sparse --dtype float16"
